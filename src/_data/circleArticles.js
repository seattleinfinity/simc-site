...
const fetchContents = async () => {
  ...
  let articlesByIssue = (
    await Promise.all(
      allIssues.map(async (issueName) => {
        ...
        let extractIssueInfo = /^([a-zA-Z]+)(\d+)$/g.exec(issueName);
        if (!extractIssueInfo) return;
        const [_, monthAbbrev, year] = extractIssueInfo;
        const monthNames = [
          'January',
          'February',
          'March',
  ...
  ).filter((article) => article); // Make sure articles are not null
